...
module.exports = {
    ...
    getMatches: async function(region, name, tag, size, filter) {
        return this.returndata(`https://api.henrikdev.xyz/valorant/v3/matches/${region}/${encodeURI(name)}/${encodeURI(tag)}${size != undefined && filter == undefined ? `?size=${size}` : size == undefined && filter != undefined ? `?filter=${filter}` : size != undefined && filter != undefined ? `?filter=${filter}&size=${size}` : ""}`, "matches")
    },
    /**
